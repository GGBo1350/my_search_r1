...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_generating_state(
        ...
        # Multi-token XML delimiters must be passed as complete stop strings;
        # treating their component IDs as independent stops truncates the XML.
        if self.tool_parser.stop_strings:
            configured_stops = sampling_params.get("stop") or []
            if isinstance(configured_stops, str):
                configured_stops = [configured_stops]
            stop_strings = list(dict.fromkeys([*configured_stops, *self.tool_parser.stop_strings]))
            sampling_params = {**sampling_params, "stop": stop_strings}
            # vLLM trims matched stop strings by default. The parser and the
            # policy both need the complete closing tag in the response.
            if self.rollout_config.name == "vllm":
                sampling_params["include_stop_str_in_output"] = True
            # SGLang exposes the equivalent behavior under this name.
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        # A single turn is one parallel batch capped at max_parallel_calls.
        # If the model emits more than the cap, execute NONE of them, append a
        # format-error tool result so the trajectory records the limit, and end
        # the turn: no search results and no answer give a hard training signal.
        exceed_cap = len(agent_data.tool_calls) > self.max_parallel_calls
        tool_call_names: list[str] = []
        if exceed_cap:
            logger.warning(
                "Terminating tool agent loop: turn contains %s tool calls, cap is %s",
                len(agent_data.tool_calls),
            ...
            responses = [
                (
                    ToolResponse(
                        text=(
                            "<information><format_error>A single turn can execute at most "
                            f"{self.max_parallel_calls} tool calls; no calls were "
                            "executed.</format_error></information>"
                        )
                    ),
                    0.0,
                    {"format_error": True},
