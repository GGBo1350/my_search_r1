...
class ServerAdapter(BaseRollout):
        ...
        if peft_config and base_sync_done:
            if self.device_mesh["infer_tp"].get_local_rank() == 0:
                ...
                req = LoadLoRAAdapterFromTensorsReqInput(
                    ...
                    # sglang 0.5.8 expects a single serialized string; with
                    # TP=1 wrap_lora_params returns a one-element list.
                    serialized_tensors=serialize_named_tensors[0],
                )
                # send http request
    ...
    def wrap_lora_params(self, peft_config: LoraConfig, weights: Generator[tuple[str, torch.Tensor]]):
        # peft config: newer verl passes peft_config.to_dict() (a plain dict);
        # older verl versions pass the LoraConfig dataclass.
        ...
        if not isinstance(task_type, str):
            peft_config_json["task_type"] = task_type.value
        peft_type = peft_config_json.get("peft_type")
        if not isinstance(peft_type, str):
            peft_config_json["peft_type"] = peft_type.value
        ...
        # ForkingPickler (MultiprocessingSerializer) serializes CPU tensors via
        # shared memory whose authkey digest is process-bound; the engine server
        # cannot deserialize it over HTTP. Standard pickle inlines the tensor
        # data, so the server's SafeUnpickler (torch.* allowed) can load it.
        import base64 as _base64
        import io as _io
        import pickle as _pickle

        buf = _io.BytesIO()
        _pickle.dump(processed_weights, buf, protocol=_pickle.HIGHEST_PROTOCOL)
        serialized_tensors = _base64.b64encode(buf.getvalue()).decode("utf-8")
        infer_tp_size = self.device_mesh["infer_tp"].mesh.size()[0]
        serialized_named_tensors = [serialized_tensors] * infer_tp_size
