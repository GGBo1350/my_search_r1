...
class CheckpointEngineManager:
    ...
    async def sleep_replicas(self):
        """Pause partial rollouts, then free rollout device memory safely."""
        # Agent-loop generation may return a completed subset while SGLang
        # still retains unfinished requests for partial rollout.  The SGLang
        # offload endpoint intentionally rejects that state.  Pause/abort first
        # so unfinished requests are saved and the scheduler is drained.
