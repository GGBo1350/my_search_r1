...
class SGLangHttpServer:
    ...
    def __init__(
        ...
        if self.model_config.lora_adapter_path is not None:
            print(
                "SGLang static LoRA model config: "
                f"rank={self.model_config.lora_rank}, "
                f"adapter={self.model_config.lora_adapter_path}, "
                f"targets={self.model_config.target_modules}"
            )
    ...
    async def wake_up(self):
        ...
        if self.rollout_mode == RolloutMode.HYBRID:
            ...
            # Restore exactly what the most recent sleep call released.  The
            # dynamic LoRA state can change during adapter synchronization, so
            # recomputing the tags here can incorrectly request a non-offloaded
            # `weights` tag and crash SGLang with KeyError.
            tags = getattr(self, "_sleep_tags", None)
            if tags is None:
                tags = ["kv_cache"] if self.lora_as_adapter else ["kv_cache", "weights"]
            obj = ResumeMemoryOccupationReqInput(tags=tags)
            await self.tokenizer_manager.resume_memory_occupation(obj, None)
            self._sleep_tags = None
