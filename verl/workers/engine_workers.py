...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if "rollout" in self.role:
            ...
            # Keep this consistent with AsyncSGLangServer.sleep(): an active
            # LoRA adapter releases only the KV cache, not the base weights.
            self.rollout.sleep_level = 1 if self.lora_as_adapter else 2
    ...
    async def update_weights(self, global_steps: int = None, mode: str = "auto"):
        ...
        # 1. Resume weights only when sleep actually released them.  In LoRA
        # adapter mode sleep_level=1 keeps base weights resident and releases
        # only the KV cache; asking SGLang to resume a non-offloaded `weights`
        # tag raises KeyError.
        if self.config.rollout.free_cache_engine and self.rollout.sleep_level == 2:
