...
class DistillationTeacherModelConfig(BaseConfig):
    ...
    lora_adapter_path (str, optional):
        Static PEFT LoRA adapter loaded on top of model_path. Currently supported
        by the SGLang teacher backend.
    lora_rank (int):
        Rank of the static teacher LoRA adapter.
    ...
    lora_rank: int = 0
    lora_target_modules: Optional[Any] = "all-linear"
    ...
    def check_configured(self):
        ...
        if self.lora_adapter_path is not None:
            ...
            if self.inference.name != "sglang":
                raise NotImplementedError("Static distillation teacher LoRA currently requires the SGLang backend.")
...
class DistillationConfig(BaseConfig):
    ...
    colocate_with_actor (bool):
        Reuse the actor resource pool for teacher inference servers. This is an
        experimental memory-sharing mode for small models on high-memory GPUs.
        Every teacher replica must have the same world size as the actor pool.
