...
class AgentLoopConfig(BaseConfig):
    num_workers: int = 8
    # Optional local CUDA device ids used by GPU-backed tools inside agent-loop
    # workers. Workers are assigned round-robin per node without reserving Ray
    # GPU resources, since colocated rollout/trainer workers own those resources.
    tool_gpu_devices: Optional[list[int]] = None
