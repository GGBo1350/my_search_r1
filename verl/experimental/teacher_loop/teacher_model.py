...
def _prepare_teacher_model_kwargs(teacher_model_config: DistillationTeacherModelConfig) -> dict:
    """Build HFModelConfig kwargs and register a static SGLang LoRA if configured."""
    kwargs = {"path": teacher_model_config.model_path}
    adapter_path = teacher_model_config.lora_adapter_path
    if adapter_path is None:
        return kwargs

    if teacher_model_config.inference.name != "sglang":
        raise NotImplementedError("Static distillation teacher LoRA currently requires SGLang.")

    from verl.workers.rollout.sglang_rollout.utils import SGLANG_LORA_NAME

    expected_lora_paths = [f"{SGLANG_LORA_NAME}={adapter_path}"]
    engine_kwargs = teacher_model_config.inference.engine_kwargs
    sglang_kwargs = dict(engine_kwargs.get("sglang", {}))
    configured_lora_paths = sglang_kwargs.get("lora_paths")
    if configured_lora_paths is not None and configured_lora_paths != expected_lora_paths:
        raise ValueError(
            f"Teacher {teacher_model_config.key!r} has conflicting SGLang lora_paths: "
            f"{configured_lora_paths!r}; expected {expected_lora_paths!r}."
        )
    sglang_kwargs["lora_paths"] = expected_lora_paths
    engine_kwargs["sglang"] = sglang_kwargs
    kwargs.update(
        {
            "lora_adapter_path": adapter_path,
            "lora_rank": teacher_model_config.lora_rank,
            "target_modules": teacher_model_config.lora_target_modules,
        }
    )
    return kwargs
...
class TeacherModelManager:
    ...
    def _initialize_llm_servers(self):
        ...
        model_config = HFModelConfig(**_prepare_teacher_model_kwargs(teacher_model_config))
        print(
            f"Teacher {teacher_model_config.key!r} resolved model config: "
            f"lora_rank={model_config.lora_rank}, "
            f"lora_adapter_path={model_config.lora_adapter_path}, "
            f"target_modules={model_config.target_modules}, "
            f"sglang_lora_paths={rollout_config.engine_kwargs.get('sglang', {}).get('lora_paths')}"
        )
...
class MultiTeacherModelManager:
    ...
    def _initialize_teacher_model_managers(self):
        ...
        if self.distillation_config.colocate_with_actor:
            required_processes = len(teacher_models) + 1  # actor + one worker group per teacher
            if required_processes > self.resource_pool.max_colocate_count:
                raise ValueError(
                    f"Colocating {len(teacher_models)} teachers with the actor requires "
                    f"max_colocate_count >= {required_processes}, got {self.resource_pool.max_colocate_count}."
                )
            incompatible = {
                key: teacher.world_size
                for key, teacher in teacher_models.items()
                if teacher.world_size != self.resource_pool.world_size
            }
            if incompatible:
                raise ValueError(
                    "Each colocated teacher must have the same world size as the actor resource pool "
                    f"({self.resource_pool.world_size}); got {incompatible}."
                )
            teacher_pools = [self.resource_pool] * len(teacher_models)
        else:
    ...
    def _sleepable_replicas(self):
        """Return teachers whose weights/KV cache may be cycled safely.

        SGLang static-LoRA teachers are read-only and small enough to remain
        resident in the colocated setup.  More importantly, releasing their
        cache immediately after prompt-logprob requests can race the scheduler's
        request teardown.  Keep those servers resident; the trainable actor
        rollout still follows its normal sleep/update-weights lifecycle.
        """
        return [
            replica
            for manager in self.teacher_model_managers.values()
            if manager.teacher_model_config.lora_adapter_path is None
            for replica in manager.rollout_replicas
        ]

    def wake_up(self) -> None:
        """Restore teacher inference memory before teacher-logprob requests."""
        if not self._is_sleeping:
            return
        _run_all([replica.wake_up() for replica in self._sleepable_replicas()])
        self._is_sleeping = False

    def sleep(self) -> None:
        """Release teacher KV caches while the colocated actor trains."""
        if self._is_sleeping:
            return
        sleepable_replicas = self._sleepable_replicas()
        if not sleepable_replicas:
            return
        _run_all([replica.sleep() for replica in sleepable_replicas])
