...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        # A single turn is one parallel batch: execute at most max_parallel_calls
        # calls.  Extra calls are not executed; each receives a normal tool
        # result carrying a format error, so the trajectory continues and the
        # model can recover on the next turn instead of being penalized.
        tasks = []
        tool_call_names = []
        ...
        if len(agent_data.tool_calls) > self.max_parallel_calls:
            logger.warning(
                "Turn contains %s tool calls, executing only %s; extra calls get a format error",
                ...
                (
                    ToolResponse(
                        text=(
                            ...
                            f"{self.max_parallel_calls} tool calls.</format_error></information>"
                        )
                    ),
                    0.0,
                    {"format_error": True},
                )
                for _ in agent_data.tool_calls[self.max_parallel_calls :]
            ]
