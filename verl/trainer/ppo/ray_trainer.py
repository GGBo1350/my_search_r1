...
def should_dump_rollout_data(global_steps: int, rollout_data_dir: Optional[str], rollout_data_freq: Any = 1) -> bool:
    """Return whether the current training step should be exported as JSONL."""
    if not rollout_data_dir:
        return False
    try:
        frequency = int(rollout_data_freq)
    except (TypeError, ValueError) as error:
        raise ValueError(f"rollout_data_freq must be a positive integer, got {rollout_data_freq!r}") from error
    if frequency <= 0:
        raise ValueError(f"rollout_data_freq must be a positive integer, got {rollout_data_freq!r}")
    return int(global_steps) % frequency == 0
...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        self.checkpoint_manager.sleep_replicas()
        if self.use_teacher_policy:
            # Teacher prompt-logprobs are only needed during rollout. Keeping
            # their KV caches resident through actor backward creates a large
            # avoidable memory peak when teachers are colocated with the actor.
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    direct_distillation_only = (
                        self.use_teacher_policy
                        and not self.distillation_config.distillation_loss.use_task_rewards
                        and not self.distillation_config.distillation_loss.use_policy_gradient
                    )
                    rollout_corr_config = self.config.algorithm.get("rollout_correction", None)
                    bypass_recomputing_logprobs = rollout_corr_config and rollout_corr_config.get("bypass_mode", False)
                    if direct_distillation_only:
                        # forward_kl_topk is backpropagated directly and does not
                        # consume old_log_probs or entropy.  Skipping this extra
                        # full-vocabulary forward avoids a large, unused memory peak.
                        pass
                    ...
                    if should_dump_rollout_data(self.global_steps, rollout_data_dir, rollout_data_freq):
