...
def _finite_scalar(value: Any) -> float | None:
    """Convert a scalar metric value to float and reject text/containers/NaN."""
...
def compute_reward_extra_metrics(
    reward_extra_infos_dict: dict[str, Any], prefix: str = "reward_extra"
) -> dict[str, float]:
    """Aggregate scalar custom-reward outputs into per-step logger metrics.

    Text and container fields such as predictions, queries and retrieved titles
    remain available for rollout dumps, but are deliberately not sent as scalar
    curves to experiment trackers.
    """
    metrics: dict[str, float] = {}
    for key, raw_values in reward_extra_infos_dict.items():
        ...
        numeric_values = [converted for value in values if (converted := _finite_scalar(value)) is not None]
        if numeric_values:
            metrics[f"{prefix}/{key}/mean"] = float(np.mean(numeric_values))
    return metrics
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                # Validation reward dictionaries may also contain predictions,
                # queries and retrieved-title lists. Only scalar numeric values
                # are valid chart metrics.
                converted_vals = [_finite_scalar(value) for value in var_vals]
                if not var_vals or any(value is None for value in converted_vals):
                    continue
                var_vals = converted_vals
