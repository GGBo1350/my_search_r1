...
def test_reward_extra_list_fields_remain_one_dimensional_across_worker_chunks_on_cpu():
    """Worker-local list regularity must not change the batch array rank."""
    dummy_worker = type(
        "_DummyWorker",
        (),
        {"reward_loop_worker_handles": object(), "distillation_enabled": False},
    )()

    def make_internal(queries: list[str], retrieved_titles: list[str]) -> _InternalAgentLoopOutput:
        return _to_internal(
            ...
            extra_fields={
                "reward_extra_info": {
                    ...
                    "retrieval_recall": float(bool(retrieved_titles)),
                }
            },
            num_turns=1,
            prompt_len=2,
    ...
    # Equal-length empty lists used to produce shape (2, 0), while the ragged
    # chunk produced shape (2,). DataProto.concat then raised ValueError.
    regular_chunk = AgentLoopWorker._postprocess(
        dummy_worker,
        inputs=[make_internal([], []), make_internal([], [])],
    )
    ragged_chunk = AgentLoopWorker._postprocess(
        dummy_worker,
        inputs=[make_internal(["alpha"], ["Alpha"]), make_internal(["beta", "gamma"], [])],
    )

    assert regular_chunk.non_tensor_batch["queries"].shape == (2,)
    assert ragged_chunk.non_tensor_batch["queries"].shape == (2,)
    merged = DataProto.concat([regular_chunk, ragged_chunk])
    assert merged.non_tensor_batch["queries"].shape == (4,)
    assert merged.non_tensor_batch["retrieved_titles"].shape == (4,)
