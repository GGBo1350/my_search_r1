#!/usr/bin/env bash
# Phase 2: train the compare-specialist teacher.
# run_teacher_compare_944.sh: Teacher-Compare: 400 compare samples, 1 epoch = 25 steps, save every 15 (2 ckpts)
# One epoch of the Teacher-Compare: 400 compare samples, 1 epoch = 25 steps, save every 15 (2 ckpts) split; general prompt kept in data (specialization comes from data only).
# Same hyperparams as Phase 1: bs16 x n8, mini 4, max_response 4096, LLM judge ON, chunked entropy.
RUN_ID=$(date +%Y%m%d_%H%M%S)
exec > /root/autodl-tmp/train_logs/qwen3_4b_teacher_compare_944_${RUN_ID}.launch.log 2>&1
...
export TRAIN_FILE=./data/hotpotqa_v3_hard_1600/train_compare_400.parquet
...
export CHECKPOINT_DIR=/root/autodl-tmp/checkpoints/qwen3_4b_teacher_compare_944_${RUN_ID}
export ROLLOUT_DATA_DIR=/root/autodl-tmp/rollouts/qwen3_4b_teacher_compare_944_${RUN_ID}
export EXPERIMENT_NAME=qwen3_4b_teacher_compare_944_${RUN_ID}
# LLM answer-equivalence judge ON
export ANSWER_LLM_JUDGE=${ANSWER_LLM_JUDGE:-1}
echo "launching run ${RUN_ID} (qwen3_4b_teacher_compare_944) at $(date)"
